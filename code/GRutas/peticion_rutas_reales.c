...
float coordenadasLudico [7][2] = { {1.7125,41.2246}, {1.7314,41.2329}, {1.7345,41.2315}, {1.7251,41.2171}, {1.72152,41.22387}, {1.72191,41.21798}, {1.72474,41.21865} };
...
float * retorna_coordenadas_barrio (int i, float *porcentaje){
...
// porcentajes de cada barrio
//mongo --eval 'db.barrios.find({},{"nombre":0, "_id":0, "densidad":0, "superficie":0,"limites":0,"habitantes":0}).pretty()' | awk '{print $4}' | tail -n +3
	...
	FILE *horarios = fopen("horarios.txt", "w");
...
/*	for(int i = 0; i < sizeof(numPersonasPorEdad)/sizeof(int); ++i){
		printf("%d \n", numPersonasPorEdad[i]);
	}*/


//	mongo --eval 'db.edades.find({}, {"edad": 0, "_id":0 } ).toArray()' | awk '{print $3}' | sed '/^\s*$/d' | tail -n +3 --> todas las edades en orden

//  Para todos los barrios
	for(int barr = 0; barr < 18; ++barr){
//  Para todas las edades
		float porcentaje;
		float * coordsBarrio = retorna_coordenadas_barrio(barr,&porcentaje);
		for(int edad = iniEdad; edad < finEdad + 1; ++edad){
			printf("------------------------------------------\n");
			printf("Edad: %d\n",edad);	
			printf("------------------------------------------\n");
			//Para la cantidad de cada edad
			for(int persona = 0; persona < (numPersonasPorEdad[edad] * porcentaje); ++persona){
				//Generacion de casa
				int esta_dentro = 0;
				float casaX;
				float casaY;
				int contX = 0;
				int contY = 0;
				float coordenadasX[sizeof(coordsBarrio)/2];
				float coordenadasY[sizeof(coordsBarrio)/2];
				for(int q = 0; q < sizeof(coordsBarrio); ++q){
					if(q%2==0){ 
				...
				if(edad<=21){ //estudiante
				...
				else if ((tasaParo * numPersonasPorEdad[edad] * porcentaje) > persona || edad >= 67){ //en el paro o jubilado
					...
					horaPrimerPaso = hora_aleatoria(7,9);
					horaSegundoPaso = hora_aleatoria(10,13);
					horaTercerPaso = arreglaHora(horaSegundoPaso + hora_aleatoria(1,2));
